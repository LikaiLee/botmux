...
import { buildMergedPendingResponseCard, buildPendingResponseCard, buildRepoSelectCard, buildStreamingCard, getCliDisplayName } from './im/lark/card-builder.js';
...
async function postPendingResponseCard(ds: DaemonSession, replyToMessageId: string, prompt: string, sender?: { name?: string }): Promise<void> {
  ...
  await pendingResponseQueue.run(ds.session.sessionId, async () => {
    ...
    if (isPendingResponseCardOpen(ds)) {
      updateMessage(ds.larkAppId, previousCardId!, buildMergedPendingResponseCard())
        .catch(err => logger.warn(`[${tag(ds)}] failed to mark previous pending response as merged: ${err instanceof Error ? err.message : String(err)}`));
    }
    const card = buildPendingResponseCard();
    try {
